...
var settings = new SLAcer.Settings({
    ...
    resin: {
        density  : 1.18, // g/cm3
        price    : 60.45,   // $
        panel    : { //look at Photocentric
            collapsed: true,
            position : 6
        }
    },
    screen: {
        width    : 1024, //resolution for Amazon Fire 7 2017
        height   : 600,
        diagonal : { size: 7, unit: 'in' },
        panel    : {
            collapsed: true,
            position : 7
        }
...
function getSlice(layerNumber) {
    ...
    // render 2D view and save to Firebase storage
    viewer2d.screenshot(function(dataURL) {
        ...
        if (PNGExport) {
            if(user){
                var fileName = 'im' + layerNumber + '.png';
                var userFolder = user.uid;
                var printFolder = (loadedFile.name).slice(0, -4);
                firebase.storage().ref('/' + userFolder + '/' + printFolder).child(fileName)
                .putString(dataURL, 'data_url', {contentType:'image/png'});
            }
            else{
                alert("You must be logged in! Go back and log in.");
                error("You must be logged in! Go back and log in.");
            }
        }
    });
}
...
var $restartButton     = $sidebar.find('#restart-button');
...
function endSlicing() {
    ...
    docRef.set({
      currentPrint: ((loadedFile.name).slice(0, -4)),
      layerNumber: currentSliceNumber
    }).then(function(){
      console.log("Saved");
...
$sliceButton.on('click', function(e) {
    ...
    $fileBody.addClass('hidden');
    $fileInput.addClass('hidden');
    startSlicing();
});

//restart button deletes the png files that have been saved to firebase storage, and clears the database of upload info
$restartButton.on('click', function(e) {
    var user = firebase.auth().currentUser;
    var pFolder = user.uid + "/" + (loadedFile.name).slice(0, -4);

    
    //delete png files from Firebase storage
    for(var i = 1; i < currentSliceNumber; i++){
        (firebase.storage().ref().child(pFolder + "/im" + i.toString() + ".png")).delete().then(function() {
          //alert("Slicing aborted");
        }).catch(function(error) {
    ...
    var firestore = firebase.firestore();
    const docRef = firestore.doc("users/" + user.uid);
    var deleteData = docRef.update({
        currentPrint: firebase.firestore.FieldValue.delete(),
        layerNumber: firebase.firestore.FieldValue.delete()
    });
